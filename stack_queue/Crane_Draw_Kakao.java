...
/*
    ...
    [ 공부해야 할 내용 ]
    1. 문제에 대한 공부 : 크레인이 인형을 집으면 그 배열의 값은 0이 되어야 함을 기억.
    2. if else 와 if 를 헷갈리지 마라. if { a내용 } else { b내용 } 과 if { a내용 } b내용 은 다르다.
       후자는 if가 있든 없든 반드시 실행시키고, 전자는 if가 아닐때만 실행시킨다.

 */

public class Crane_Draw_Kakao {
    public static int solution(int n, int[][] board, int m, int[] moves) {
        int answer = 0;
        Stack<Integer> stack = new Stack<>();
        for ( int choice : moves ) {
            for ( int i=0; i<n; i++ ) {
                int pick = board[i][choice-1];
                if ( pick != 0 ) {
